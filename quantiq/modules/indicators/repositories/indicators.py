...
class IndicatorRepository:
    ...
    def fetch(self, stock_id: int) -> Indicator | None:
        with transaction() as conn:
            cursor = conn.cursor()
            row = cursor.execute("SELECT stock_id, p_l, p_vp, p_ebit, psr, p_ativos, p_cap_giro, p_ativ_circ_liq, div_yield, ev_ebitda, ev_ebit, cres_rec_5a FROM indicators WHERE stock_id = ?", (stock_id,)).fetchone()
