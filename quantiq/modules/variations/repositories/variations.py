...
from quantiq.modules.variations.domains.entities import Variation, Variations
...
class VariationsRepository:
    ...
    def store(self, variations: Variations) -> None:
        with transaction() as conn:
            try:
                values = [
                    (variation.stock_id, variation.period, variation.value)
    ...
    def fetch(self, stock_id: int) -> Variations:
        with transaction() as conn:
            cursor = conn.cursor()
            rows = cursor.execute(
                "SELECT stock_id, period, value FROM variations WHERE stock_id = ?",
                (stock_id,),
            ).fetchall()
